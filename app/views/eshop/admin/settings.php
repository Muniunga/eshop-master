...
<form method="post" enctype="multipart/form-data">
	<table class="table table-striped table-advance table-hover">
		...
		<?php elseif($type == "slider_images"):?>
			...
			<?php if($action == "show"):?>
				<thead>
				<tr><th>Titulo</th><th>Subtitulo</th><th>Texto</th><th>Link do Produto</th><th>Produto Imagem</th><th>Desativado</th><th>Action</th></tr>
				</thead>
				<tbody>
					<?php if(isset($rows) && is_array($rows)):?>
						<?php foreach($rows as $row):?>
							...
							<tr>
								...
								<td><?=$row->disabled ? "Sim":"Não";?></td>
							</tr>
						<?php endforeach;?>
					<?php endif;?>
				</tbody>
				<a href="<?=ROOT?>admin/settings/slider_images?action=add">
					<input type="button" value="Adicionar" class="btn btn-warning pull-right" >
				</a>
