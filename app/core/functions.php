...
function is_paid($order)
{
	...
	if (is_array($payment)) {
		return "<button class='btn btn-success'>Pago</button>";
	}

	return "<button class='btn btn-warning'>Não Pago</button>";
}
...
function get_product_count()
{
	...
	$DB = Database::newInstance();
	$ROWS = $DB->read("select description from products p join (select sum(qty), productid from order_details group by productid
	order by sum(qty) desc) itemMaisVendido on (p.id=itemMaisVendido.productid)");

	if (is_array($ROWS)) {

		$result = array_column($ROWS, 'result');
